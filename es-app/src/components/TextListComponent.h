...
public:
	...
	inline void
	setCursorChangedCallback(const std::function<void(CursorState state)> &func) { mCursorChangedCallback = func; }
	...
	for (int i = startEntry; i < listCutoff; i++) {
		...
		if (!entry.data.textCache)
			entry.data.textCache = std::unique_ptr<TextCache>(
					font->buildTextCache(mUppercase ? Utils::String::toUpper(entry.name) : entry.name, 0, 0,
										 0x000000FF));
...
void TextListComponent<T>::update(int deltaTime) {
	...
	if (!isScrolling() && size() > 0) {
		...
		if (textLength > limit) {
			...
			mMarqueeOffset = (int) (Math::Scroll::loop(delay, scrollTime + returnTime, (float) mMarqueeTime,
...
template<typename T>
void TextListComponent<T>::applyTheme(const std::shared_ptr<ThemeData> &theme, const std::string &view,
	...
	if (properties & ALIGNMENT) {
		if (elem->has("alignment")) {
			...
			else LOG(LogError) << "Unknown TextListComponent alignment \"" << str << "\"!";
		}
