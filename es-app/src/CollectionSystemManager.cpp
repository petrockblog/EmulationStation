...
CollectionSystemManager::CollectionSystemManager(Window* window) : mWindow(window)
{
	CollectionSystemDecl systemDecls[] = {
		...
		{ AUTO_ALL_GAMES,       "all",          "all games",         "filename, a.ending",      "auto-allgames",           false },
		{ AUTO_LAST_PLAYED,     "recent",       "last played",       "last played, de.ending",  "auto-lastplayed",         false },
		{ AUTO_FAVORITES,       "favorites",    "favorites",         "filename, a.ending",      "auto-favorites",          false },
		{ CUSTOM_COLLECTION,    myCollectionsName,  "collections",    "filename, a.ending",      "custom-collections",      true }
	};
	...
	for (std::vector<CollectionSystemDecl>::const_iterator it = tempSystemDecl.begin(); it != tempSystemDecl.end(); ++it )
	{
...
CollectionSystemManager::~CollectionSystemManager()
{
	...
	// iterate the map
	for(std::map<std::string, CollectionSystemData>::const_iterator it = mCustomCollectionSystemsData.begin() ; it != mCustomCollectionSystemsData.end() ; it++ )
	{
...
void CollectionSystemManager::saveCustomCollection(SystemData* sys)
{
	...
	if (found) {
		...
		if (sysData.needsSave)
		{
			...
			for(std::unordered_map<std::string, FileData*>::const_iterator iter = games.begin(); iter != games.end(); ++iter)
			{
...
{
	for(std::map<std::string, CollectionSystemDecl>::const_iterator it = mCollectionSystemDeclsIndex.begin() ; it != mCollectionSystemDeclsIndex.end() ; it++ )
	{
