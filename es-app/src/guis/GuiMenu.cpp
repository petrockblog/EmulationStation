...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, _("MAIN MENU").c_str()), mVersion(window)
{
	...
	addEntry(_("UI SETTINGS").c_str(), 0x777777FF, true,
		[this] {
			...
			auto screensaver_behavior = std::make_shared< OptionListComponent<std::string> >(mWindow, _("SCREENSAVER BEHAVIOR").c_str(), false);
			std::vector<std::string> screensavers;
			screensavers.push_back(_("dim"));
			screensavers.push_back(_("black"));
			screensavers.push_back(_("random video"));
			...
			// show filtered menu
			row.elements.clear();
			row.addElement(std::make_shared<TextComponent>(mWindow, _("VIDEO SCREENSAVER SETTINGS").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
			...
			transitions.push_back(_("instant"));
			for(auto it = transitions.begin(); it != transitions.end(); it++)
			...
			styles.push_back(_("automatic"));
			styles.push_back(_("basic"));
			styles.push_back(_("detailed"));
			styles.push_back(_("video"));
	...
	addEntry(_("GAME COLLECTION SETTINGS").c_str(), 0x777777FF, true,
		[this] { openCollectionSystemSettings();
		});

	addEntry(_("OTHER SETTINGS").c_str(), 0x777777FF, true,
		[this] {
			...
			modes.push_back(_("disabled"));
			modes.push_back(_("default"));
			modes.push_back(_("enhanced"));
			modes.push_back(_("instant"));
	...
	addEntry(_("CONFIGURE INPUT").c_str(), 0x777777FF, true,
		[this] {
			Window* window = mWindow;
			window->pushGui(new GuiMsgBox(window, _("ARE YOU SURE YOU WANT TO CONFIGURE INPUT?").c_str(), _("YES").c_str(),
				[window] {
	...
	addEntry(_("QUIT").c_str(), 0x777777FF, true,
		[this] {
			...
			row.makeAcceptInputHandler([window] {
				window->pushGui(new GuiMsgBox(window, _("REALLY RESTART?").c_str(), _("YES").c_str(),
				[] {
					if(quitES("/tmp/es-restart") != 0)
						LOG(LogWarning) << "Restart terminated with non-zero result!";
				}, _("NO").c_str(), nullptr));
			});
			row.addElement(std::make_shared<TextComponent>(window, _("RESTART EMULATIONSTATION").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
			s->addRow(row);
			...
			row.addElement(std::make_shared<TextComponent>(window, _("RESTART SYSTEM").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
			s->addRow(row);

			row.elements.clear();
			row.makeAcceptInputHandler([window] {
				window->pushGui(new GuiMsgBox(window, _("REALLY SHUTDOWN?").c_str(), _("YES").c_str(),
				[] {
					if(quitES("/tmp/es-shutdown") != 0)
						LOG(LogWarning) << "Shutdown terminated with non-zero result!";
				}, _("NO").c_str(), nullptr));
			});
			row.addElement(std::make_shared<TextComponent>(window, _("SHUTDOWN SYSTEM").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
			s->addRow(row);
			...
			if(Settings::getInstance()->getBool("ShowExit"))
			{
				row.elements.clear();
				row.makeAcceptInputHandler([window] {
					window->pushGui(new GuiMsgBox(window, _("REALLY QUIT?").c_str(), _("YES").c_str(),
					[] {
						SDL_Event ev;
						ev.type = SDL_QUIT;
						SDL_PushEvent(&ev);
				...
				row.addElement(std::make_shared<TextComponent>(window, _("QUIT EMULATIONSTATION").c_str(), Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
				s->addRow(row);
			}
	...
	mVersion.setText(_("EMULATIONSTATION V").c_str() + strToUpper(PROGRAM_VERSION_STRING));
...
void GuiMenu::openScreensaverOptions() {
	mWindow->pushGui(new GuiScreensaverOptions(mWindow, _("VIDEO SCREENSAVER").c_str()));
}
